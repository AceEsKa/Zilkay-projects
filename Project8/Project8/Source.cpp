...
double average(const initializer_list<int>& data) 
{
    ...
    double sum=std::accumulate(data.begin(),data.end(),0.0)/ data.size();
...
{
    // TODO
    std::list<int>::const_iterator it = std::find(data.begin(),data.end(),value);

    return it; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
{
    // TODO
    std::list<int>::const_iterator it = std::find_if(data.begin(), data.end(), isInRange200to400);
...
{
    // TODO
    //std::transform(data.begin(), data.end(), data, increment);
}
...
{
    // TODO
    /*std::list<int> l(data1.size());
    auto it_b = data1.begin();
    auto it_e = data1.begin();
    std::advance(it_e, data1.size());
    std::transform(it_b,it_e, data2, l, std::plus<int>());*/

    return std::list<int>(); // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
{
    // TODO
    std::sort(data.rbegin(), data.rend(), std::less<int>());
}
...
int main() {
    ...
    std::vector<int> v = {4,10,3,7,9,6,2,5,1,0,8};
    sort2(v);
    for (auto it : v)
    {
    ...
    //std::initializer_list<int> l = { 2,3 };
    //double in = std::accumulate(l.begin(), l.end(), 0) / l.size();
    //std::cout << in << endl;
    return 0;
}